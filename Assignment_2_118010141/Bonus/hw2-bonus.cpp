...
void *print_map(void *t){
	while (!stop){
		...
		puts(map[ROW]);
		printf(RESET);
		printf("\n/*          Little Frog Crosses River           */\n");
		printf("\n[W]:up [S]:down [A]:left [D]:right\n");
		printf("[J]:Speed up [K]:Speed down\n");
		printf("\nCurrent speed: %d",10 - sleep_time/LEVEL);
...
void *logs_move(void *t){
	...
	int log_length = rand()%11 + 8; // 8-18 
	int speed_buff = ((rand()%3) * LEVEL) - LEVEL/2; // -5000 0 5000
	...
	while (!flag && !stop){
		if (kbhit()){
			...
			if (dir=='w' || dir=='W'){
				...
				if (sleep_time>LEVEL) sleep_time -= LEVEL;
			}
			else if (dir=='k' || dir=='K'){
				if (sleep_time<9*LEVEL) sleep_time += LEVEL;
			}
			else if (dir=='q' || dir=='Q'){
				flag = QUIT;
			}
